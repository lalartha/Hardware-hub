...
export default function ManageRequests() {
    ...
    const loadRequests = async () => {
        ...
        try {
            ...
            let query = supabase
                .from('requests')
                .select('*, borrower:profiles!user_id(id, name, email), hardware:hardware_items(id, name, category)')
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row gap-8">
                <Card className="flex-1 border border-border bg-card shadow-sm rounded-[2rem] p-8 flex items-center gap-8 group">
                    <div className="h-16 w-16 rounded-2xl bg-blue-500/10 border-2 border-blue-500/10 flex items-center justify-center text-blue-500 group-hover:scale-110 transition-transform">
                        <History size={32} />
                    </div>
                    <div>
                        <h4 className="text-lg font-black tracking-tight text-foreground mb-1">Usage Insights</h4>
                        <p className="text-sm text-muted-foreground font-medium leading-relaxed max-w-sm">
                            Analyze borrow rates and demand patterns across your hardware listing portfolio.
                        </p>
                    </div>
                </Card>
                <Card className="flex-1 border border-border bg-card shadow-sm rounded-[2rem] p-8 flex items-center gap-8 group">
                    <div className="h-16 w-16 rounded-2xl bg-amber-500/10 border-2 border-amber-500/10 flex items-center justify-center text-amber-500 group-hover:scale-110 transition-transform">
                        <AlertCircle size={32} />
                    </div>
                    <div>
                        <h4 className="text-lg font-black tracking-tight text-foreground mb-1">Inventory Health</h4>
                        <p className="text-sm text-muted-foreground font-medium leading-relaxed max-w-sm">
                            Flag technical issues or maintenance requirements during the hardware return workflow.
                        </p>
                    </div>
                </Card>
            </div>
